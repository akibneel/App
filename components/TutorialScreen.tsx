  ...
  return (
    <div className="p-6 animate-in slide-in-from-right-10 duration-500">
      <div className="flex items-center gap-3 mb-6 px-1">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight leading-none">Tutorial</h2>
          <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mt-1">Learn to Earn</p>
        </div>
      </div>
      ...
      <div className="relative group mb-8 cursor-pointer" onClick={() => handleWatchVideo(config.heroVideoUrl)}>
        <div className="w-full aspect-video bg-slate-900 rounded-[32px] overflow-hidden shadow-2xl relative border-4 border-white">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-6 text-center">
            ...
            <h3 className="font-semibold text-lg tracking-tight mb-1">Watch Official Guide</h3>
            <p className="text-[10px] text-white/60 font-semibold uppercase tracking-widest">Master the TakaEarn process</p>
          </div>
          ...
          <div className="absolute bottom-4 left-4 right-4 flex justify-between items-center">
            ...
            <span className="bg-red-600 text-[8px] font-semibold px-2 py-0.5 rounded-full text-white flex items-center gap-1">
      ...
      {/* Roadmap Section */}
      <div className="mb-8">
        <h3 className="text-[11px] font-semibold text-slate-900 uppercase tracking-widest mb-5 flex items-center gap-2">
            ...
            <div key={idx} className="bg-white p-5 rounded-[32px] shadow-sm border border-slate-50 flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300" style={{ animationDelay: `${idx * 100}ms` }}>
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold text-slate-800 text-[13px] tracking-tight">{step.title}</h4>
              ...
              {step.buttonText && step.buttonUrl && (
                <button 
                  ...
                  className="w-full bg-slate-50 hover:bg-emerald-50 text-slate-600 hover:text-emerald-700 py-3 rounded-2xl flex items-center justify-center gap-2 text-[10px] font-semibold uppercase tracking-widest border border-slate-100 hover:border-emerald-200 transition-all active:scale-[0.98]"
                >
                  <PlayCircle size={14} />
                  {step.buttonText}
