...
const AdminLoginScreen: React.FC<AdminLoginScreenProps> = ({ credentials, onLoginSuccess, onBack }) => {
  ...
  return (
    <div className="min-h-full bg-slate-950 flex flex-col p-8 animate-in fade-in duration-500 overflow-hidden relative">
      ...
      <button onClick={onBack} className="self-start p-3 bg-white/5 rounded-2xl text-white/40 hover:text-white hover:bg-white/10 transition-all mb-12 border border-white/5">
        <ArrowLeft size={20} />
      </button>
      ...
      <div className="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full">
        <div className="flex flex-col items-center mb-10">
          ...
          <h2 className="text-2xl font-black text-white tracking-tight text-center">Restricted Access</h2>
          <p className="text-[10px] font-black text-white/30 uppercase tracking-[4px] mt-2">Authorized Personnel Only</p>
        </div>

        <form onSubmit={handleAdminLogin} className="space-y-4">
          <div className="space-y-2">
            <label className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-1">Admin Identity</label>
            <div className="relative group">
              ...
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 p-5 pl-12 rounded-3xl text-white font-bold focus:outline-none focus:border-emerald-500 focus:bg-white/10 transition-all"
          ...
          <div className="space-y-2">
            <label className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-1">Secure Key</label>
            <div className="relative group">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-emerald-500 transition-colors">
                <Lock size={18} />
              </div>
          ...
          {error && (
            <div className="bg-rose-500/10 border border-rose-500/20 p-4 rounded-2xl flex items-center gap-3 animate-in shake duration-500">
              ...
              <p className="text-rose-500 text-[10px] font-black uppercase tracking-wider">Invalid Admin Credentials</p>
            </div>
          )}
          ...
          <button 
            type="submit"
            disabled={isLoading}
            className="w-full bg-white text-slate-900 py-6 rounded-3xl font-black text-[12px] uppercase tracking-[3px] shadow-[0_0_30px_rgba(255,255,255,0.1)] active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50 mt-4"
          >
            {isLoading ? (
        ...
        <p className="text-[9px] text-white/20 text-center font-bold uppercase tracking-widest mt-12 leading-relaxed px-4">
