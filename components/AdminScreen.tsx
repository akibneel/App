...
const AdminScreen: React.FC<AdminScreenProps> = ({ 
  ...
  const handleDownloadFile = (dataUrl: string, userName: string, taskTitle: string, openInTab: boolean = false) => {
    if (openInTab) {
                ...
                return (
                  <div key={sub.id} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[32px] p-5 shadow-sm transition-colors flex flex-col gap-5">
                      ...
                      <div className="flex items-center justify-between mb-3">
                         ...
                         <div className="flex items-center gap-2">
                            {sub.screenshot && isImg && (
                              <button onClick={() => setViewingScreenshot(sub.screenshot!)} className="text-emerald-600 text-[9px] font-black uppercase flex items-center gap-1"><ImageIcon size={12} /> View</button>
                            )}
                            {sub.screenshot && (
                              <button onClick={() => handleDownloadFile(sub.screenshot!, sub.userName, sub.taskTitle)} className="text-blue-600 text-[9px] font-black uppercase flex items-center gap-1"><Download size={12} /> Download</button>
                            )}
                         </div>
                      </div>
                    ...
                    <div className="flex gap-2">
                       ...
                       <button onClick={() => onAction(sub.id, tabType === 'LEADS' ? TaskStatus.RECEIVED : TaskStatus.APPROVED, adj.quantity)} className={`flex-1 ${tabType === 'LEADS' ? 'bg-blue-600' : 'bg-emerald-600'} text-white h-11 rounded-2xl shadow-lg font-bold text-[10px] uppercase`}>{tabType === 'LEADS' ? 'Receive' : 'Approve'}</button>
                    </div>
                  </div>
                );
              })}
  ...
  const FinanceManagement = () => {
    ...
    return (
      <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
        <div className="bg-white dark:bg-slate-900 p-7 rounded-[40px] border border-slate-100 dark:border-slate-800">
           ...
           <div className="space-y-4">
              ...
              <button onClick={() => onUpdateAppConfig({ ...appConfig, minWithdrawal: parseFloat(minWithdrawal) || 0 })} className="w-full bg-slate-900 dark:bg-emerald-600 text-white py-5 rounded-3xl font-bold text-[11px] uppercase tracking-[3px]">Update Setting</button>
           </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {appConfig.withdrawalMethods.map(m => (
            <div key={m.id} className="bg-white dark:bg-slate-900 p-4 rounded-[32px] border border-slate-100 dark:border-slate-800 flex items-center justify-between">
              ...
              <span className="text-[10px] font-black dark:text-white uppercase">{m.name}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
        <div className="bg-white dark:bg-slate-900 p-7 rounded-[40px] border border-slate-100 dark:border-slate-800 space-y-4">
           ...
           <div className="space-y-3">
              ...
              <button onClick={() => onUpdateTutorialConfig({...tutorialConfig, heroVideoUrl: heroVideo, supportTelegram: supportTele, telegramChannel: channelTele})} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-bold text-[10px] uppercase">Save Changes</button>
           </div>
        </div>
        <div className="space-y-3">
           {tutorialConfig.steps.map(step => (
             <div key={step.id} className="bg-white dark:bg-slate-900 p-5 rounded-[32px] border border-slate-100 dark:border-slate-800 flex items-center gap-4">
               <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-950 flex items-center justify-center text-xl">{renderIcon(step.icon)}</div>
               <div className="flex-1 truncate"><h5 className="text-[12px] font-bold dark:text-white truncate">{step.title}</h5></div>
             </div>
           ))}
        </div>
      </div>
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 min-h-screen p-4 transition-colors">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={onBack} className="p-2.5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl dark:text-slate-100 shadow-sm"><ArrowLeft size={22} /></button>
        <div className="flex flex-col">
          <h2 className="text-[17px] font-black dark:text-slate-100 uppercase">Admin Core</h2>
          <p className="text-[8px] font-black text-slate-400 uppercase tracking-[3px]">System Management</p>
        </div>
      </div>
      ...
      {activeTab === 'OVERVIEW' && (
        <div className="grid grid-cols-2 gap-4 animate-in fade-in duration-500">
           <div className="bg-slate-900 p-6 rounded-[40px] text-white overflow-hidden relative border border-transparent dark:border-slate-800">
               <h3 className="text-4xl font-black mb-1">{analytics.totalSignups}</h3>
           ...
           <div className="bg-emerald-600 p-6 rounded-[40px] text-white overflow-hidden relative">
               <h3 className="text-4xl font-black mb-1">{analytics.currentlyOnline}</h3>
      ...
      {activeTab === 'TASKS' && (
        <div className="space-y-6">
           <button onClick={() => { resetForm(); setShowTaskForm(true); }} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase">New Entry</button>
           {showTaskForm && (
             <div ref={formRef} className="bg-white dark:bg-slate-900 p-7 rounded-[40px] shadow-2xl border border-slate-100 dark:border-slate-800 space-y-5">
               <div className="flex gap-5 items-center">
                  ...
                  <div className="flex-1 space-y-3">
                     <input type="text" placeholder="Task Name" value={formTask.title} onChange={e => setFormTask({...formTask, title: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl text-[12px] font-black dark:text-white border-none" required />
                     <input type="text" placeholder="Unit (e.g. ID)" value={formTask.rateType} onChange={e => setFormTask({...formTask, rateType: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl text-[12px] font-black dark:text-white border-none" required />
                  </div>
               </div>
               <div className="grid grid-cols-2 gap-4">
                  <input type="number" step="0.1" value={formTask.rate} onChange={e => setFormTask({...formTask, rate: e.target.value})} className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl text-[12px] font-black dark:text-white" required />
                  <input type="number" value={formTask.limit} onChange={e => setFormTask({...formTask, limit: e.target.value})} className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl text-[12px] font-black dark:text-white" required />
               </div>
               <button onClick={handleSaveTask} className="w-full bg-slate-900 dark:bg-emerald-600 text-white py-6 rounded-3xl font-black text-[12px] uppercase">Deploy Configuration</button>
             </div>
           )}
           <div className="space-y-4">
             {tasks.map(task => (
               <div key={task.id} className="bg-white dark:bg-slate-900 p-5 rounded-[40px] border border-slate-100 dark:border-slate-800 flex items-center gap-5">
                 <div className={`${task.color} w-16 h-16 rounded-[28px] flex items-center justify-center shrink-0 text-3xl overflow-hidden`}>{renderIcon(task.icon)}</div>
                 <div className="flex-1 truncate"><h5 className="text-[14px] font-black dark:text-white truncate">{task.title}</h5><p className="text-[9px] font-black text-slate-400 uppercase tracking-[2px] mt-1">৳{task.rate} • Cap {task.limit}</p></div>
                 <div className="flex gap-2"><button onClick={() => handleEditTask(task)} className="p-3.5 bg-slate-50 dark:bg-slate-800 rounded-2xl"><Edit3 size={18} /></button><button onClick={() => onDeleteTask(task.id)} className="p-3.5 bg-slate-50 dark:bg-slate-800 rounded-2xl"><Trash2 size={18} /></button></div>
               </div>
             ))}
           </div>
        </div>
      ...
      {activeTab === 'SECURITY' && (
        <div className="bg-slate-950 dark:bg-slate-900 rounded-[40px] p-10 text-white relative animate-in slide-in-from-bottom-4 transition-colors">
          <h3 className="text-xl font-black tracking-tight mb-10 flex items-center gap-4"><Lock size={24}/> Admin Access</h3>
          <form onSubmit={handleUpdateSecurity} className="space-y-6">
            <div className="space-y-2">
               <label className="text-[9px] font-black text-white/40 uppercase tracking-[4px]">Identity Username</label>
            ...
            <button type="submit" className="w-full bg-white text-slate-900 py-6 rounded-3xl text-[12px] font-black uppercase tracking-[5px]">Commit Security Updates</button>
          </form>
        </div>
      )}
